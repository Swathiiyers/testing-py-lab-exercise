...
def most_and_least_common_type(treats):
    ...
    >>> treats = [{'type': 'dessert'},{'type': 'dessert'},{'type': 'appetizer'},{'type': 'appetizer'},{'type': 'drink'}, {'type': 'fruit'}]
    >>> most_and_least_common_type(treats)
    ('dessert', 'drink')
...
def get_treats():
    """Return treats being brought to the party.
        for memory address use => # doctest: +ELLIPSIS
        to combine ellipsis and whitespace use new comment on the next line

    One day, I'll move this into a database! -- Balloonicorn
    >>> treats = [{ 'type': 'dessert', 'description': 'Chocolate mousse', 'who': 'Leslie'}, {'type': 'dessert','description': 'Cardamom-Pear pie','who': 'Joel'}]
    >>> get_treats  # doctest: +ELLIPSIS
    <function get_treats at 0x...>
